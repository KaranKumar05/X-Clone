...
function Gork() {
  document.title = `X - Grok`;

  const [selectedOption, setSelectedOption] = useState("Fun Mode");
  const [isOpen, setIsOpen] = useState(false);

  const options = ["Fun Mode", "Regular Mode"];
  ...
  return (
    <div className="flex items-start justify-start gap-0 tablet:justify-center pl-0 mobile:pl-8 tablet:gap-4">
      ...
      <span className="w-[100%] mobile-landscape:w-[37rem]">
        <div className="w-100% tablet:[38rem] text-white border-r border-l h-[85vh] mobile:h-[100vh] sticky top-0 left-0 right-0 flex flex-col justify-between border-gray-600 mb-20 mobile:mb-0">
          <header className=" flex flex-col items-center backdrop-blur-md bg-black/10 z-10 sticky top-0 left-0 right-0" >
            <span className="flex items-center w-[100%] font-medium">
              <div className="text-xl px-2 w-1/3">
                <FaChevronLeft className="cursor-pointer" onClick={prevPage} />
              </div>
              <div className="w-1/2 flex items-center gap-2  py-3 text-center">
                <div className="relative">
                  <button
                    onClick={() => setIsOpen(!isOpen)}
                    className="py-2 px-4 bg-transparent rounded-full focus:outline-none flex items-center gap-2 w-[12rem] justify-center hover:bg-[#16181C]"
                  >
                    {selectedOption === "Fun Mode" ? (
                      <HiMiniEyeSlash className="text-xl" />
                    ) : (
                      <BsFillSlashSquareFill className="text-xl" />
                    )}
                    {selectedOption}
                    <FaAngleDown />
                  </button>
                  {isOpen && (
                    <div className="absolute mt-1 bg-black shadow-[0_0px_10px] shadow-gray-500 rounded-xl w-[12rem] ">
                      {options.map((option) => (
                        <div
                          key={option}
                          onClick={() => handleSelectOption(option)}
                          className="px-4 py-2 cursor-pointer hover:bg-[#16181C] rounded-md"
                        >
                          {option}
                        </div>
                      ))}
          ...
          <div className="flex justify-center flex-col items-center gap-3 p-8 sticky top-0 left-0 right-0">
            <h1 className="text-3xl font-bold">Grok someting</h1>
            <p className="text-gray-500 text-center">
              Premium+ subscribers can now use our most advance AI, Grok, on X.
            </p>
            <button className="bg-white text-black font-bold px-8 py-2 rounded-full w-[100%]  mobile:w-fit">
              Subscribe now
            </button>
          </div>
        </div>
